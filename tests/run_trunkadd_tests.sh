...
cd "$TMP"
./zpaqfranz trunkadd compress dummy-source --ec-data 16 --ec-stripes 8 >/dev/null
./zpaqfranz trunkadd compress dummy-source --ec-data 16 --ec-stripes 8 >/dev/null
...
rm compress.ecstate
./zpaqfranz trunkadd compress dummy-source --ec-data 16 --ec-stripes 8 | grep -q 'recovered missing .ecstate once'
[ -f compress.003 ] && [ -f compress.003.ec ]
grep -q 'last_part=3' compress.ecstate
echo "TRUNKADD TESTS PASS"
