...
C="$ROOT/src/oec_cold.hpp"
grep -Fq 'OECPAR1' "$C"
grep -Fq 'OECOLD1' "$C"
grep -Fq 'zfec::fopen_utf8' "$C"
! grep -Eq '_findfirst\s*\(|_findnext\s*\(|_spawnv\s*\(' "$C"
grep -Fq "shutil.copy2(here / 'oec_cold.hpp'" "$ROOT/scripts/apply_to_upstream.py"
echo "OEC COLD STATIC TESTS PASS"
