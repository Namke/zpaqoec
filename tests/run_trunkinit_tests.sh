...
TMP="$ROOT/tests/trunkinit-test-run"
...
./zpaqfranz trunkinit 'compress.???' --ec-data 16 --ec-stripes 8 >/dev/null
...
if ./zpaqfranz trunkinit 'compress.???' >/dev/null 2>&1; then echo 'expected refusal without --force'; exit 1; fi
# Force rebuild is transactional and recreates all EC.
./zpaqfranz trunkinit 'compress.???' --force --ec-data 16 --ec-stripes 8 >/dev/null
./zpaqfranz ec verify compress.000 >/dev/null
...
./zpaqfranz trunkinit 'legacy_????.zpaq' --ec-data 16 --ec-stripes 8 >/dev/null
[ -f legacy_0000.zpaq ] && [ -f legacy_0000.zpaq.ec ]
[ -f legacy_0001.zpaq.ec ] && [ -f legacy_0002.zpaq.ec ]
echo 'TRUNKINIT TESTS PASS'
