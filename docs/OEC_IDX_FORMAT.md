...
`OECIDX1` stores the materialized default `l` and `i` views. These sections are plaintext. For a standard AES-encrypted ZPAQ zero-part, OEC 0.3.7 therefore disables plaintext IDX creation and automatic use by default. The durable encrypted archive, `.000`, and `.ec` files remain unaffected.
...
An `.idx` created by 0.3.1 for an encrypted archive may already contain plaintext metadata. 0.3.7 will not use it automatically unless `--idx-plaintext` is supplied; `oec_idx drop` can remove it.
...
## Current acceleration coverage (0.3.7)
