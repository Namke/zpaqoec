...
0.4.0 does **not** replace zpaqfranz's native in-memory `Jidac`/`HTIndex` structures during `a`. Native `oec_a` still reconstructs the file/fragment state from `.000` and uses upstream dedup logic. Likewise `oec_x/oec_e` do not yet use a structured IDX fragment->part/block locator to bypass native multipart extraction.
