...
#if defined(_WIN32) || defined(__MINGW32__) || defined(__MINGW64__)
 struct _stat64 st;if(_stat64(p.c_str(),&st)!=0)return -1;return (int64_t)st.st_mtime;
#else
...
inline bool fingerprint(const std::string& source,Fingerprint& fp,std::string& err){if(!zfec::get_file_size(source,fp.size)){err="cannot stat OEC zero-part index: "+source;return false;}fp.mtime=file_mtime(source);FILE*f=std::fopen(source.c_str(),"rb");if(!f){err="cannot open OEC zero-part index: "+source;return false;}uint64_t mid=fp.size>kSampleBytes?fp.size/2:0,last=fp.size>kSampleBytes?fp.size-kSampleBytes:0;bool ok=sample_crc(f,0,fp.size,fp.first)&&sample_crc(f,mid,fp.size,fp.middle)&&sample_crc(f,last,fp.size,fp.last);std::fclose(f);if(!ok){err="cannot fingerprint OEC zero-part index: "+source;return false;}return true;}
...
inline bool atomic_replace(const std::string&tmp,const std::string&dst,std::string&err){const std::string bak=dst+".oecidx.bak";bool had=zfec::file_exists(dst);if(had){std::remove(bak.c_str());if(std::rename(dst.c_str(),bak.c_str())!=0){err="cannot move old idx aside: "+dst;return false;}}if(std::rename(tmp.c_str(),dst.c_str())!=0){if(had)std::rename(bak.c_str(),dst.c_str());err="cannot install idx: "+dst;return false;}if(had)std::remove(bak.c_str());return true;}
...
inline bool write_cache_v2(const std::string&idx_path,const std::string&source,const std::string&list_text,const std::string&info_text,const std::vector<FileInput>&inputs,std::string&err){
 ...
 uint64_t off=sizeof(h);h.list_offset=off;h.list_size=list_text.size();h.list_crc32c=zfec::crc32c(list_text.data(),list_text.size());off+=h.list_size;h.info_offset=off;h.info_size=info_text.size();h.info_crc32c=zfec::crc32c(info_text.data(),info_text.size());off+=h.info_size;h.file_offset=off;h.file_count=files.size();h.file_record_size=sizeof(FileRecord);h.file_crc32c=zfec::crc32c(files.data(),files.size()*sizeof(FileRecord));off+=files.size()*sizeof(FileRecord);h.string_offset=off;h.string_size=pool.size();h.string_crc32c=zfec::crc32c(pool.data(),pool.size());off+=pool.size();h.path_hash_offset=off;h.path_hash_count=hashes.size();h.path_hash_record_size=sizeof(PathHashRecord);h.path_hash_crc32c=zfec::crc32c(hashes.data(),hashes.size()*sizeof(PathHashRecord));off+=hashes.size()*sizeof(PathHashRecord);h.created_unix=(uint64_t)std::time(0);h.header_crc32c=header_crc_v2(h);
 const std::string tmp=idx_path+".tmp";std::remove(tmp.c_str());FILE*f=std::fopen(tmp.c_str(),"wb");if(!f){err="cannot create idx temp file: "+tmp;return false;}bool ok=std::fwrite(&h,1,sizeof(h),f)==sizeof(h);if(ok&&!list_text.empty())ok=std::fwrite(list_text.data(),1,list_text.size(),f)==list_text.size();if(ok&&!info_text.empty())ok=std::fwrite(info_text.data(),1,info_text.size(),f)==info_text.size();if(ok&&!files.empty())ok=std::fwrite(files.data(),sizeof(FileRecord),files.size(),f)==files.size();if(ok&&!pool.empty())ok=std::fwrite(pool.data(),1,pool.size(),f)==pool.size();if(ok&&!hashes.empty())ok=std::fwrite(hashes.data(),sizeof(PathHashRecord),hashes.size(),f)==hashes.size();if(ok)ok=std::fflush(f)==0;if(std::fclose(f)!=0)ok=false;if(!ok){std::remove(tmp.c_str());err="cannot write idx temp file: "+tmp;return false;}return atomic_replace(tmp,idx_path,err);
}
inline bool write_cache_v1_legacy(const std::string&idx_path,const std::string&source,const std::string&list_text,const std::string&info_text,std::string&err){
 Fingerprint fp;if(!fingerprint(source,fp,err))return false;HeaderV1 h;std::memset(&h,0,sizeof(h));std::memcpy(h.magic,"OECIDX1",7);h.version=1;h.header_size=sizeof(h);h.flags=(list_text.empty()?0:kFlagList)|(info_text.empty()?0:kFlagInfo);h.source_size=fp.size;h.source_mtime=fp.mtime;h.source_crc_first=fp.first;h.source_crc_middle=fp.middle;h.source_crc_last=fp.last;h.list_offset=sizeof(h);h.list_size=list_text.size();h.list_crc32c=zfec::crc32c(list_text.data(),list_text.size());h.info_offset=h.list_offset+h.list_size;h.info_size=info_text.size();h.info_crc32c=zfec::crc32c(info_text.data(),info_text.size());h.created_unix=(uint64_t)std::time(0);h.header_crc32c=header_crc_v1(h);const std::string tmp=idx_path+".tmp";std::remove(tmp.c_str());FILE*f=std::fopen(tmp.c_str(),"wb");if(!f){err="cannot create legacy idx temp";return false;}bool ok=std::fwrite(&h,1,sizeof(h),f)==sizeof(h);if(ok&&!list_text.empty())ok=std::fwrite(list_text.data(),1,list_text.size(),f)==list_text.size();if(ok&&!info_text.empty())ok=std::fwrite(info_text.data(),1,info_text.size(),f)==info_text.size();if(std::fclose(f)!=0)ok=false;if(!ok){std::remove(tmp.c_str());err="cannot write legacy idx temp";return false;}return atomic_replace(tmp,idx_path,err);
}
...
inline bool remove_cache(const std::string&path,std::string&err){if(!zfec::file_exists(path))return true;if(std::remove(path.c_str())!=0){err="cannot remove idx: "+path;return false;}return true;}
