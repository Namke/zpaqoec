...
#if defined(_WIN32) || defined(__MINGW32__) || defined(__MINGW64__)
 FILE*f=std::fopen(path.c_str(),"r+b");if(!f){err="cannot open idx for deep resize";return false;}int fd=_fileno(f);int rc=_chsize_s(fd,n);std::fclose(f);if(rc){err="cannot resize idx for deep table";return false;}return true;
#else
 if(::truncate(path.c_str(),(off_t)n)!=0){err="cannot resize idx for deep table";return false;}return true;
#endif
}
inline bool read_h2(const std::string&path,oecidx::HeaderV2&h,std::string&err){FILE*f=std::fopen(path.c_str(),"rb");if(!f){err="cannot open idx header";return false;}bool ok=std::fread(&h,1,sizeof(h),f)==sizeof(h);std::fclose(f);if(!ok||std::memcmp(h.magic,"OECIDX2",7)!=0||h.version!=2){err="deep backend requires OECIDX2";return false;}return true;}
inline bool write_h2(const std::string&path,oecidx::HeaderV2 h,std::string&err){h.header_crc32c=oecidx::header_crc_v2(h);FILE*f=std::fopen(path.c_str(),"r+b");if(!f){err="cannot update idx header";return false;}bool ok=std::fwrite(&h,1,sizeof(h),f)==sizeof(h)&&std::fflush(f)==0;std::fclose(f);if(!ok)err="cannot write idx header";return ok;}
