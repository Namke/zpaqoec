...
Assert-OecOutput -Label 'oec_version dispatcher' -CommandArgs @('oec_version') -Needle 'zpaqoec OEC overlay 0.3.6'
...
if (-not [string]::IsNullOrWhiteSpace($InstallTo)) {
  ...
  $BuiltExe = $Installed
  try {
    Assert-OecOutput -Label 'installed oec_version dispatcher' -CommandArgs @('oec_version') -Needle 'zpaqoec OEC overlay 0.3.6'
  } finally {
    $BuiltExe = $SavedBuilt
  }
}
